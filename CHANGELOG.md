...
### Features
- **mcp:** `SendMessage` tool on the headroom MCP server. Sends a message to another Claude Code session on this machine over that session's local messaging socket (Claude Code's own peer protocol, version 1), or lists the sessions that are reachable. For hosts such as the Claude desktop app that switch off the built-in `SendMessage`; an idle recipient starts a turn on the message, and replies arrive natively. Granted to the code agent as a managed allow rule.
...
### Fixed
- **proxy/aging:** tool-result aging now retires old tool results in doubling rungs (`HEADROOM_AGING_FIRST_BATCH_TOKENS`, default 128,000 candidate tokens; then 256k, 512k, ...) instead of fixed 10,000-token batches (`HEADROOM_AGING_BATCH_TOKENS`, removed). Every change to the aged set rewrites the provider's cached prefix, so the fixed batch size re-sent a 300k-token Claude Code conversation at the cache-write price every three to six turns: measured on 2026-09-06, 27 of the 36 full prefix rewrites across every session followed an aging step, costing 8.2M tokens of cache writes (69% of all writes that night) to save about 2M base-priced tokens of reads. With doubling rungs the number of rewrites grows logarithmically with the conversation, the first rewrite is sized to pay for itself, and aged blocks still never flip back to raw text.
